  ...
  const loadAll = useCallback(async () => {
    ...
    const { data: appts } = await supabase
      ...
      const [{ data: estAll }, { data: estSigned, error: estSignedError }, { data: estThisMonth }, { data: estLastMonth }, { data: pendingInvoices, error: pendingInvoicesError }] = await Promise.all([
        ...
        supabase.from('invoices').select('id,invoice_number,amount,invoice_type,estimate_id').eq('user_id', user.id.toString().toLowerCase().trim().replace(/[^\x20-\x7E]/g, '')).eq('status', 'pending').eq('invoice_type', 'standard').limit(10),
      ])
