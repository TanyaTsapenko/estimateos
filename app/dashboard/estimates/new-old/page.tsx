...
const DEFAULT_OPENING: Omit<Opening, 'id'> = {
  ...
  pane: 'double', egress_required: false, shape_position: '',
}
...
function OpeningCard({ op, idx, customOpeningTypes, customPrices, palette, openingsCount, removeOpening, updateOpening, duplicateOpening, userId, typeFieldVisibility, subtypesByType }: OpeningCardProps) {
  ...
  return (
    <div className="op-card">
        ...
        <div style={{ paddingTop: 6 }}>
          <div className="r2" style={{ marginBottom: 8 }}>
            <div className="f"><label>Shape</label>
              <select value={op.shape} onChange={e => { updateOpening(op.id, 'shape', e.target.value); if (e.target.value !== 'arch') updateOpening(op.id, 'shape_position', '') }} disabled={op.type === 'window_combo'} style={{ opacity: op.type === 'window_combo' ? 0.4 : 1, cursor: op.type === 'window_combo' ? 'not-allowed' : 'pointer' }}>
            ...
            {op.shape === 'arch' && (
              <div className="f"><label>Arch Position</label>
                <select value={op.shape_position || 'full'} onChange={e => updateOpening(op.id, 'shape_position', e.target.value)}>
                  <option value="full">Full Arch</option>
                  <option value="half">Half Arch</option>
                  <option value="quarter">Quarter Arch</option>
                </select></div>
            )}
            {!hasPalette && (
      ...
      {openGroup.glass && (
        <div style={{ paddingTop: 6 }}>
          <div className="f" style={{ marginBottom: 8 }}><label>Glazing</label>
            <div style={{ display: 'flex', gap: 6 }}>
              {(['double', 'triple'] as const).map(p => (
                <button key={p} type="button"
                  onClick={() => updateOpening(op.id, 'pane', p)}
                  style={{ flex: 1, padding: '9px 0', borderRadius: 8, border: `1.5px solid ${(op.pane || 'double') === p ? '#2563EB' : '#E2E5EA'}`, background: (op.pane || 'double') === p ? '#EFF4FF' : '#fff', fontSize: 13, fontWeight: 600, color: (op.pane || 'double') === p ? '#2563EB' : '#64748B', cursor: 'pointer', fontFamily: 'inherit' }}>
                  {p === 'double' ? 'Double Pane' : `Triple Pane${customPrices?.surcharges?.triple_pane ? ` (+$${customPrices.surcharges.triple_pane})` : ''}`}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 8 }}>
            ...
            {op.type.startsWith('window_') && (
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#374151', cursor: 'pointer', userSelect: 'none' }}>
                <input type="checkbox" checked={op.egress_required || false} onChange={e => updateOpening(op.id, 'egress_required', e.target.checked)}
                  style={{ width: 15, height: 15, accentColor: '#2563EB', cursor: 'pointer' }} />
                Egress Required{customPrices?.surcharges?.egress_required ? ` (+$${customPrices.surcharges.egress_required})` : ''}
              </label>
            )}
          </div>
        </div>
...
function NewEstimateForm() {
      ...
      if (editId) {
        ...
        if (ops && ops.length > 0) {
          setOpenings(ops.map((op: any) => ({
            ...
            pane: (op as any).pane || 'double',
            egress_required: Boolean((op as any).egress_required),
            shape_position: (op as any).shape_position || '',
          })))
        }
      } else if (apptId) {
  ...
  async function saveEstimate() {
    ...
    const rows = openings.map((op, i) => ({
      ...
      pane: op.pane || 'double',
      egress_required: op.egress_required || false,
      shape_position: op.shape_position || null,
