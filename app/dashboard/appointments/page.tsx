                ...
                <div key={appt.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--jet)', marginBottom: 1 }}>
                        {appt.client_name}
                      </div>
                      {appt.client_phone && (
                        <div style={{ fontSize: 11, color: 'var(--ash)' }}>{appt.client_phone}</div>
                      )}
                    </div>
                    <span style={{
                      background: sm.bg, color: sm.color, fontSize: 9, fontWeight: 700,
                      padding: '3px 8px', borderRadius: 8, letterSpacing: '.06em', flexShrink: 0, marginLeft: 8,
                    }}>
                  ...
                  {/* Address */}
                  {appt.client_address && (
                    <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 6, display: 'flex', alignItems: 'flex-start', gap: 3 }}>
                      <span>📍</span>
                      <span>{appt.client_address}</span>
                    </div>
                  )}

                  {/* Time + lead source */}
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: appt.notes ? 8 : 10 }}>
                    {appt.appointment_time && (
                      <span style={{ fontSize: 12, color: 'var(--jet)', fontWeight: 600 }}>
                        🕐 {fmt12h(appt.appointment_time)}
                      </span>
                    )}
                    {appt.lead_source && (
                      <span style={{ fontSize: 11, color: 'var(--ash)' }}>
                        {LEAD_ICONS[appt.lead_source] || '📌'} {appt.lead_source}
                      </span>
                    )}
                    {appt.assigned_to && (
                      <span style={{ fontSize: 11, color: 'var(--ash)' }}>
                  ...
                  {appt.notes && (
                    <div style={{
                      fontSize: 11, color: '#6b7280', background: '#F4F5F7', borderRadius: 8,
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {appt.client_phone && (
                      <a href={`tel:${appt.client_phone}`}
                        style={{
                          flex: 1, background: 'rgba(37,99,235,.08)', borderRadius: 8, padding: '8px 0',
                          fontSize: 11, fontWeight: 700, color: '#2563eb', textAlign: 'center',
                    ...
                    {appt.client_address && (
                      <a href={`https://maps.google.com/?q=${encodeURIComponent(appt.client_address)}`}
                        ...
                        style={{
                          flex: 1, background: 'rgba(22,163,74,.08)', borderRadius: 8, padding: '8px 0',
                          fontSize: 11, fontWeight: 700, color: '#16a34a', textAlign: 'center',
                          textDecoration: 'none', display: 'block',
                        }}>
                        🗺 Map
                      </a>
                    ...
                    {!appt.estimate_id && appt.status !== 'cancelled' && (
                      <button onClick={() => createEstimate(appt)}
                        style={{
                          flex: 2, background: 'rgba(59,108,255,.1)', border: 'none', borderRadius: 8,
                          padding: '8px 0', fontSize: 11, fontWeight: 700, color: 'var(--blue-dark)',
                    ...
                    {appt.estimate_id && (
                      <button onClick={() => router.push(`/dashboard/estimates/${appt.estimate_id}`)}
                        style={{
                          flex: 2, background: 'rgba(147,51,234,.1)', border: 'none', borderRadius: 8,
                          padding: '8px 0', fontSize: 11, fontWeight: 700, color: '#9333ea',
                          cursor: 'pointer', fontFamily: 'inherit',
                        }}>
                        📋 View Estimate
                      </button>
                    ...
                    <button onClick={() => deleteAppt(appt.id)}
                      style={{
                        width: 34, background: 'rgba(220,38,38,.06)', border: 'none', borderRadius: 8,
