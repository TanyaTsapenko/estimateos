  ...
  return (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', borderTop: '1px solid rgba(10,22,40,0.07)', padding: '12px 20px' }}>
      <div style={{ maxWidth: 820, margin: '0 auto', display: 'flex', gap: 12 }}>
        <button onClick={onDiscard} disabled={!!saving} style={{ flex: 1, height: 52, borderRadius: 12, border: '1px solid #e5e7eb', background: '#fff', color: saving ? '#94A3B8' : '#475467', fontSize: 15, fontWeight: 500, cursor: saving ? 'default' : 'pointer', fontFamily: 'inherit' }}>
          Discard
        </button>
        <button onClick={onSave} disabled={!valid || !!saving} style={{ flex: 1, height: 52, borderRadius: 12, border: 'none', background: valid && !saving ? '#2563EB' : '#93aef5', color: '#fff', fontSize: 15, fontWeight: 600, cursor: valid && !saving ? 'pointer' : 'default', fontFamily: 'inherit' }}>
...
export default function CompanySettingsPage() {
  ...
  if (role === 'estimator') return (
    <div style={{ minHeight: '100vh', background: '#F8F9FB', fontFamily: 'Inter, sans-serif' }}>
      <AppTopBar onBack={() => router.back()} backLabel="Settings" title="Company" />
