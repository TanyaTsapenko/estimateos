...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [{ data: openings, error: opErr }, { data: company, error: compErr }, { data: priceRows }, { data: subtypeRows }] = await Promise.all([
      ...
      admin.from('profiles').select('company_name, first_name, last_name, email, phone, address, city, province, postal, website, licence, gst_hst_number, wsib_number, logo_url, warranty_summary').eq('id', estimate.user_id).maybeSingle(),
