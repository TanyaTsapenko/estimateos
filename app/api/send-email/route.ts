...
<div style="max-width:520px;margin:0 auto;padding:28px 16px">
  ...
  <div style="background:#fff;border-radius:0 0 16px 16px;padding:28px">
    ...
    <div style="background:#F0FDF4;border:1px solid rgba(5,150,105,.2);border-radius:12px;padding:18px;margin-bottom:24px;text-align:center">
      ...
      <div style="font-size:11px;color:#94A3B8;margin-top:6px">inc. ${taxLabel}${est.signed_at ? ` · Signed ${new Date(est.signed_at).toLocaleDateString('en-CA', { month: 'long', day: 'numeric', year: 'numeric' })}` : ''}</div>
    </div>
  ...
  <p style="text-align:center;font-size:10px;color:#9ca3af;margin-top:16px">
    ...
    const bodyText = isContractOnly
      ...
      : isEstimateContract
        ...
        : `<strong style="color:#0A1628">${companyName}</strong> has prepared an estimate for your project. Review the details and sign online — it takes less than a minute.`
...
<div style="max-width:520px;margin:0 auto;padding:28px 16px">
  ...
  <div style="background:#fff;border-radius:0 0 16px 16px;padding:28px">
    ...
    ${!isContractOnly ? `<div style="background:#F4F5F7;border-radius:12px;padding:18px;margin-bottom:20px;text-align:center">
      ...
      <div style="font-size:12px;color:#94A3B8;margin-top:6px">inc. ${taxLabel} · Valid until ${est.valid_until || '30 days'}</div>
    </div>` : ''}
