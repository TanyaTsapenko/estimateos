...
export async function GET(request: NextRequest) {
  ...
  const { data: prof } = await supabase
    .from('profiles')
    .select('company_name, address, city, province, postal, phone, email, website, licence, insurance, logo_url, deposit_percent, contract_intro, contract_terms, contract_require_sign, contract_show_licence, signature_url, contractor_signature_url')
    .eq('id', est.user_id)
    .single()
...
<div class="hdr">
  <div class="hdr-top">
    <div class="hdr-company">
      ...
      ${((prof as any)?.address || prof?.city) ? `<div class="hdr-company-sub">${[(prof as any)?.address, prof?.city, [(prof as any)?.postal, prof?.province].filter(Boolean).join(' ')].filter(Boolean).join(', ')}</div>` : ''}
