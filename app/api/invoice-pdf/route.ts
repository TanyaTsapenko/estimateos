...
export async function GET(request: NextRequest) {
  ...
  const [{ data: est }, { data: prof }] = await Promise.all([
    ...
    supabase.from('profiles')
      .select('company_name, first_name, last_name, email, phone, city, province, website, licence, insurance, logo_url')
      .eq('id', user.id)
      .single(),
  ])
...
<div class="footer">Generated by EstimateOS · estimateos.ca</div>
