...
export async function POST(request: NextRequest) {
  ...
  const { error: emailError } = await resend.emails.send({
    ...
    html: buildHtml(linkData.properties.action_link),
  })

  if (emailError) {
    console.error('[register] Resend error:', emailError.message)
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:20px;overflow:hidden">
      ...
      <tr><td style="background:#080E1C;padding:28px 24px 36px">
        ...
        <div style="font-size:26px;font-weight:800;color:#ffffff;font-family:Arial,sans-serif;line-height:1.2">Confirm your email</div>
      </td></tr>
