...
export const F: Record<string, FieldDef> = {
  ...
  bayAngleCustom: { label: 'Specify bay angle', kind: 'text', sec: 'basics', ph: 'e.g. 35°' },
  customShapeDesc: { label: 'Describe the shape', kind: 'text', sec: 'basics', ph: 'e.g. arched top with 24″ radius', visibleWhen: { field: 'shape', value: 'Custom Shape' } },
  ...
  glassType2:  { label: 'Glass finish',      kind: 'select', sec: 'look',
    ...
    types: {
      ...
      doubleEntry: { name: 'Double entry', subs: ['Equal double','Unequal double',
        ...
        extraFieldsBySubtype: {
          ...
          'Double + Sidelites + Transom':   ['sidelights','transomAbove'],
        } },
...
export function makeOpening(typeId: string, subIndex?: number): Opening {
  ...
  const op: Opening = { typeId, sub: (subIndex != null && t.subs[subIndex] != null) ? t.subs[subIndex] : '', tempId: crypto.randomUUID(), vals }
  console.log('makeOpening called, subIndex:', subIndex, 'sub result:', op.sub)
